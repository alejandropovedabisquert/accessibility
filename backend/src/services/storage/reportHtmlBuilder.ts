...
type ReportItem = ScanResults['violations'][number];
...
const SECTIONS: Array<{ key: ReportSection; label: string; detailsId: string }> = [
    { key: 'violations', label: 'Violations', detailsId: 'violations-details' },
    { key: 'passes', label: 'Passes', detailsId: 'passes-details' },
    { key: 'incomplete', label: 'Incomplete', detailsId: 'incomplete-details' },
    { key: 'inapplicable', label: 'Inapplicable', detailsId: 'inapplicable-details' },
];
...
const blockDetails = (item: ReportItem): string => {
    ...
    return `
        <article class="rule-item">
            ...
            <p><strong>Impact:</strong> <span class="impact-pill ${impactClass}">${item.impact ? escapeHtml(item.impact) : 'Undefined'}</span></p>
            <p><strong>Help description:</strong> ${escapeHtml(item.help)}</p>
            <p><strong>Help url:</strong> <a href="${escapeHtml(item.helpUrl)}" target="_blank" rel="noopener noreferrer" class="summary-link">${escapeHtml(item.helpUrl)}</a></p>
                ...
                ${nodes.length > 0 ? nodes.map((node) => `
                    <li>
                        ...
                        <div class="target-list">
                            ${(node.target ?? []).map((selector) => `<code class="selector-code">${escapeHtml(selector)}</code>`).join('') || '<span>No target selector available</span>'}
                        </div>
                        ${node.failureSummary ? `<p><strong>Failure Summary:</strong> ${escapeHtml(node.failureSummary)}</p>` : ''}
                        <hr />
                    </li>
        ...
        return `
            <div class="summary-card">
                <div class="summary-card-header">
                    <h3 class="summary-card-title">Scan Distribution</h3>
                    <span class="summary-total">Total checks: ${escapeHtml(total)}</span>
                </div>
                <div class="summary-chart" role="img" aria-label="Distribution chart by result type">
...
const detailsSection = (report: ReportData, key: ReportSection, label: string, detailsId: string): string => {
    ...
    return `
        <section class="report-section">
            <h2 id="${detailsId}" class="section-title">${label} Details</h2>
...
export function reportHtmlBuilder(report: ReportData): string {
    return `
        <html>
            <head>
                <meta charset="UTF-8" />
            ...
            <body>
                <main class="report-shell">
                    <header class="report-header">
                        <h1 class="page-title">Accessibility Scan Results</h1>
                        ...
                        <p class="timestamp"><strong>Timestamp:</strong> ${new Date(escapeHtml(report.timestamp)).toLocaleString()}</p>
                    </header>

                    <h2 id="summary" class="section-title">Summary</h2>
                    ${blockSummary(report)}
                    ${SECTIONS.map(({ key, label, detailsId }) => detailsSection(report, key, label, detailsId)).join('')}
                </main>
            </body>
        </html>
    `;
